...
function addArtist(array,nameValue,yearsValue,genreValue,nationalityValue,bioValue) {
  const cloneArray = [...array]
...
cloneArray.push(addingMe)
return(cloneArray)
}

console.log(addArtist(artists,'Nikki','1997 - August 26','Web Design','Hispanic','My name is Nikki. i like tacos.'))
...
function lotsOfArt(array) {
  const greaterThanOneHundred = array.filter(artists => artists.paintings > 100)
 let artistNames = greaterThanOneHundred.map(artists => artists.name);
 return artistNames;
}
console.log(lotsOfArt(artists))

//console.log(lotsOfArt)
//look at array then find the artists with over 100 paintings using the "paintings"  then list those artists
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 8: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
...
function artistInfo(array,artistName) {
  const oneArtist = array.filter(artists => artists.name === artistName)
  const artistBio = oneArtist.map(artists => artists.bio);
  return artistBio[0];
}
...
function artistByCountry(array,nationality){
 const findingArtists = array.filter(artists => artists.nationality === nationality);
 const artistNationality = findingArtists.map(artists => artists.name);
